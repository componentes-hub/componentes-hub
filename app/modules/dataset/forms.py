...
class CompModelForm(FlaskForm):
    comp_filename = StringField("Comp Filename", validators=[DataRequired()])
    ...
    def get_fmmetadata(self):
        return {
            "comp_filename": self.comp_filename.data,
            "title": self.title.data,
            "description": self.desc.data,
            "publication_type": self.publication_type.data,
            "publication_doi": self.publication_doi.data,
            "tags": self.tags.data,
            "comp_version": self.version.data,
        }
...
class DataSetForm(FlaskForm):
    ...
    tags = StringField("Tags (separated by commas)")
    authors = FieldList(FormField(AuthorForm))
    comp_models = FieldList(FormField(CompModelForm), min_entries=1)
    ...
    def get_comp_models(self):
        return [fm.get_comp_model() for fm in self.comp_models]
