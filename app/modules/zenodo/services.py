...
from app.modules.compmodel.models import CompModel
...
class ZenodoService(BaseService):
    ...
    def create_new_deposition(self, dataset: DataSet) -> dict:
        ...
        metadata = {
            ...
            "keywords": (
                ["componenteshub"] if not dataset.ds_meta_data.tags else dataset.ds_meta_data.tags.split(", ") + ["componenteshub"]
            ),
            "access_right": "open",
            "license": "CC-BY-4.0",
        }
    ...
    def upload_file(self, dataset: DataSet, deposition_id: int, comp_model: CompModel, user=None) -> dict:
        """
        Upload a file to a deposition in Zenodo.

        Args:
            deposition_id (int): The ID of the deposition in Zenodo.
            comp_model (CompModel): The CompModel object representing the comp model.
            user (CompModel): The User object representing the file owner.
        ...
        data = {"name": comp_filename}
        user_id = current_user.id if user is None else user.id
        file_path = os.path.join(uploads_folder_name(), f"user_{str(user_id)}", f"dataset_{dataset.id}/", comp_filename)
        files = {"file": open(file_path, "rb")}
